...
public class Writer2QML {
	...
	// Beinhaltet das Zofar QML Konvertierungsergebnis
	private String zofarQmlFileContentString;
	...
	private void disposeImages() {
		...
		if (iconReload != null)
			iconReload.dispose();
		if (iconOkEnabled != null)
			iconOkEnabled.dispose();
		if (iconOkDisabled != null)
			iconOkDisabled.dispose();
	}

	// MenuItems
	MenuItem miExportMessages;
	MenuItem miExportQml;
	MenuItem miExportZofarQml;
	...
	// StyledText für Zofar QML Ausgabe in Tab3
	StyledText zofarQmlText;
	...
	ExportZofarQmlSelected exportZofarQmlSelected = new ExportZofarQmlSelected();
	...
	public Writer2QML() {
		...
		// Export Zofar QML
		miExportZofarQml = new MenuItem(exportMenu, SWT.PUSH);
		miExportZofarQml.setText("Zofar QML Ausgabe..");
		miExportZofarQml.addSelectionListener(exportZofarQmlSelected);
		miExportZofarQml.setEnabled(false);
		...
		// Icons
		try {
			appIcon = new Image(shell.getDisplay(),
					new FileInputStream(appDir + fs + "images" + fs + "writer2qml-32x32.png"));
			iconOpen = new Image(shell.getDisplay(),
					new FileInputStream(appDir + fs + "images" + fs + "fileOpen-24x24.png"));
			iconReload = new Image(shell.getDisplay(),
					new FileInputStream(appDir + fs + "images" + fs + "reload-24x24.png"));
			iconOkEnabled = new Image(shell.getDisplay(),
					new FileInputStream(appDir + fs + "images" + fs + "grnball-10x10.gif"));
			iconOkDisabled = new Image(shell.getDisplay(),
					new FileInputStream(appDir + fs + "images" + fs + "gryball-10x10.gif"));
		...
		zofarQmlTab = new TabItem(tabFolder, SWT.NONE);
		zofarQmlTab.setText(" Zofar QML Ausgabe");
		zofarQmlTab.setImage(iconOkDisabled);
		// zofarQmlTab
		...
		// Textfeld für Anzeige der QML-Datei im QML-Tab
		zofarQmlText = new StyledText(tabFolder, SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER);
		zofarQmlText.setEditable(false);
		zofarQmlText.addLineStyleListener(xmlLineStyleListener);
		zofarQmlTab.setControl(zofarQmlText);
	...
	public class XmlLineStyleListener implements LineStyleListener {
		...
		public void lineGetStyle(LineStyleEvent event) {
			...
			while (elementGtMatcher.find()) {
				...
				style.fontStyle = SWT.BOLD;
				// styles.add( style );
				// System.out.println("start: " + (lineOffset + elementGtMatcher.start()) +
				// "len: " + (elementGtMatcher.end() - elementGtMatcher.start()));
			}
	...
	final class ExportMessagesSelected implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			// DateiName
			if (currentFile != null)
				dlg.setFileName(currentFile.getName() + ".txt");
	...
	final class ExportZofarQmlSelected implements SelectionListener {
		public void widgetDefaultSelected(SelectionEvent arg0) {
		}
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			// Titel
			dlg.setText("Zofar QML Datei Exportieren");

			// Dateifilter
			String[] s = new String[2];
			s[0] = "Zofar QML Dateien (*.xml)";
			s[1] = "Alle Dateien (*.*)";
			dlg.setFilterNames(s);
			...
			// DateiName
			if (currentFile != null)
				dlg.setFileName("questionnaire_" + FileClient.getInstance().getNameWithoutSuffix(currentFile) + ".xml");
			...
			if (fileName != null) {
				...
				if (zofarQmlFileContentString != null) {
					...
					// Existiert Datei?
					if (!outFile.exists()) {
						Utilities.writeStringtoIsoFile(fileName, zofarQmlFileContentString);
					}
					// Datei existiert bereits
					else {
						// Dialog zum Nachfragen
						...
						// if( res == SWT.YES ) Utilities.writeStringtoIsoFile( fileName,
						// zofarQmlFileContentString);
						if (res == SWT.YES)
							try {
								FileClient.getInstance().writeToFile(fileName, zofarQmlFileContentString, false);
							} catch (IOException e) {
	...
	final class ExportQmlSelected implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			// DateiName
			if (currentFile != null)
				dlg.setFileName(currentFile.getName() + ".xml");
			...
			if (fileName != null) {
				...
				if (qmlFileContentString != null) {
					...
					// Datei existiert bereits
						...
						if (res == SWT.YES) {
							try {
								FileClient.getInstance().writeToFile(fileName, qmlFileContentString, false);
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			if (dlgDir != null) {
				...
				if (currentFile != null) {
					...
					/*
					 * Dialog zum Nachfragen MessageBox mb = new MessageBox( shell,
					 * SWT.ICON_QUESTION | SWT.YES | SWT.NO ); mb.setText( "Datei Überschreiben?" );
					 * mb.setMessage( "Die Datei " + destHtmlFileName + "existiert bereits." +
					 * "möchten Sie die Datei Überschreiben?"); int res = mb.open(); if( res ==
					 * SWT.YES ){}
					 * 
					 */
	...
	final class HelpSelected implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			// Hilfeseite laden
			// helpBrowser.setText("<html><body><h2>Writer2QML</h2>Hilfe
			// ...</body></html>");
	...
	public class UpdateHtmlViewRunnable implements Runnable {
		...
		/*
		 * Ergebnis richtig prüfen ! Endlsoviech vermeiden !
		 */

		public void run() {

			// Transformation gibt result-Datei plus Dateien index.html [ + Dateien
			// out2.html bis outN.html ] aus
			...
			ProcessBuilder builder = new ProcessBuilder("java", "-jar", "saxon9.jar", "-o", "out.null", "in.xml",
					"in.xsl");
	...
	public void convertWriterFile(String fileName) {
		...
		if (!shell.getText().equals(titleText))
		...
		// messageText.replaceStyleRanges( 0, messageText.getCharCount()-1, new
		// StyleRange[ 0 ] );
		 ...
		 *
		 * - read paragraphs - check paragraphs - convert paragraphs to qstn - convert
		 * qstn to simple_qml - convert simple_qml to simle_html_questionnaire
		 *
		 * - NEW 1: convert paragraphs to Zofar-QML - NEW 2: convert QML to clickable
		 * Zofar questionnaire
		 *
		 */
		...
		/**
		 * convert paragraph list to zofar qml (the convert method is not implemented
		 * yet )
		 **/
		if (paragraphListIsValid) {
			try {
				final String convertedZofarQml = Paragraphs2ZofarQmlConverter.convertToZofarQml(textParagraphList);
				zofarQmlFileContentString = convertedZofarQml;
				zofarQmlText.setText(zofarQmlFileContentString);
		...
		// q&d, move codingError property and method W2qOodataReader.checkParStyles() to
		// TextParagraphList
		if (paragraphListIsValid) {
		...
		if (paragraphListIsValid) {
			...
			// Model2QmlConverter converter = new Model2QmlConverter( handler.getQstn(), new
			// File( xmlFileName ));
			...
			// Zofar UI aktualisieren
			zofarQmlTab.setImage(iconOkEnabled);
			miExportZofarQml.setEnabled(true);
		...
		/*
		 * Im folgenden werden die Fehler ausgegeben: wie funktioniert das Quelle sind
		 * zwei Listen. codingErrors enthält Fehlerobjekte ( Zeile, Fehlermeldung )
		 * writerContentBuffer enthält Absatzobjekte ( Zeile, Typ und Text ) Die Fehler
		 * werden als TreeNodes in einem Tree, die Absätze als Zeilen in StyledText
		 * ausgegeben
		 *
		 ...
		 * Fehler verbunden ist (Fehler.Absatznummer = Absatz.nummer), wir ein
		 * "Fehlerknoten" in den Tree eingefügt. Der Knoten enthält ein Integer
		 * Datenobjekt mit der Zeilennummer des Fehlers im StyledText. Die entsprechende
		 * Zeile im StyledText wird markiert.
		 *
		 * Aus Performancegründen wird das häufige Aktualisieren des StyledText
		 * vermieden.Der Text wird in einem Stringbuffer, die Styleranges in einer
		 * Arraylsit zwischengesprichert und am Ende aktualisiert
		 *
		 */
		...
		while (parIt.hasNext()) {
			...
			if (currentError != null) {
				...
				if (currentError.parNumber == parCounter) {
					...
					/*
					 * int lineNumber = messageText.getLineCount()-3; if (lineNumber < 0 )lineNumber
					 * = 0;
					 * 
					 * //Achtung testen!! int lineLength; if( messageText.getLineCount() >
					 * lineNumber ){ lineLength = messageText.getOffsetAtLine( lineNumber + 1 ) -
					 * messageText.getOffsetAtLine( lineNumber ); }else{ lineLength =
					 * messageText.getCharCount() - messageText.getOffsetAtLine( lineNumber ); }
					 * 
					 * sr = new StyleRange(); sr.start = messageText.getOffsetAtLine( lineNumber );
					 * sr.length = lineLength; sr.background = bgSelected; sr.foreground =
					 * display.getSystemColor( SWT.COLOR_WHITE ); messageText.setStyleRange( sr );
		...
		// messageText.setStyleRanges( (StyleRange[]) styleList.toArray( new StyleRange[
		// 0 ]) );
